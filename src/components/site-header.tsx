...
export function SiteHeader({ extraActions }: SiteHeaderProps) {
  ...
  useEffect(() => {
    ...
    sync();
    window.addEventListener("linkano_cart_updated", sync);
    window.addEventListener("storage", sync);
    return () => {
      window.removeEventListener("linkano_cart_updated", sync);
      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3 sm:px-6">
          <Link to="/" className="flex shrink-0 items-center gap-2.5">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-600 text-lg font-black text-white shadow-md">
              L
            </div>
            <div className="hidden sm:block">
              <p className="text-2xl font-extrabold leading-none tracking-tight text-slate-950">Linkano</p>
              <p className="mt-0.5 flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider text-gold-700">
                <ShieldCheck className="h-3 w-3" /> Compras sem Fronteira
              </p>
            </div>
          </Link>

          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-full border-2 border-brand-500/80 bg-white pl-4 pr-1.5 py-1 shadow-sm transition focus-within:border-brand-600 focus-within:ring-2 focus-within:ring-brand-200">
            ...
            />
            <Button onClick={handleSearch} className="hidden h-8 shrink-0 rounded-full bg-brand-600 px-5 text-xs font-bold hover:bg-brand-700 sm:inline-flex">
              Pesquisar
            </Button>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            {extraActions}
            <Button asChild variant="ghost" className="hidden text-slate-600 hover:text-brand-700 lg:inline-flex">
        ...
        <nav className="scrollbar-none border-t border-brand-100 bg-brand-50/60 px-4 py-2 sm:px-6">
          <div className="mx-auto flex max-w-7xl items-center gap-2 overflow-x-auto">
            <Link
              to="/marketplace"
              className="inline-flex shrink-0 items-center gap-1 rounded-full bg-brand-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm"
            >
            ...
            {categories.map((cat) => (
              <Link
                ...
                className="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-white hover:text-brand-700 hover:shadow-sm"
              >
                {cat}
              </Link>
            ))}
